# Amanda Ross - Information Base

**Last Updated**: 2026-02-15
**Quick Status**: Needs Improvement — pattern of missed tasks and poor communication on blockers continues despite expectations session. Flash report missed week of 2026-02-13. CEO remains frustrated.

---

## 1. Performance Status

**Overall Rating**: Needs Improvement
**Trajectory**: Mixed — receptive to feedback and complying with floor presence directive, but new concerns emerged same week (exam bed, missed flash report, medication delivery)
**Last Formal Review**: N/A
**Next Review Due**: TBC

**Current Concerns**:
- Recurring forgetfulness / lack of attention to detail — CEO frustrated, multiple examples — first noted 2026-02-10, status: ongoing
- Missed flash report week of 2026-02-13 (was off Friday, did not send before leaving) — first noted 2026-02-13, status: needs addressing
- Failed to escalate blocker on exam bed task (too heavy to move, didn't communicate) — first noted 2026-02-13
- Delivery management — medications received and stored without communicating to anyone, not first occurrence — first noted 2026-02-13, status: ongoing
- Sits at reception instead of working the floor — expectation set 2026-02-11, currently complying

**Recent Wins** (last 30 days):
- Flash report submitted week of 2026-02-06: strong content (Saba onboarding, invoicing rollout, heating fix)
- Complying with floor presence directive — not sitting at reception as requested (2026-02-13)
- Nursing roster supplied on time (2026-02-11)
- Monday 8am meeting organised when CEO in town (2026-02-11)
- Invoicing system rollout — nurses completing treatment invoices, increased same-day payments (2026-02-06)
- Saba onboarding — week one induction completed (2026-02-06)
- Daily walkthrough checklist implemented and maintained (2026-02-10)

---

## 2. Interaction Summary

| Type | Last Date | Summary |
|------|-----------|---------|
| 1:1 | 2026-02-11 | Expectations-setting session — positives acknowledged, floor presence, snag list, Asana work board introduced |
| Flash Report | 2026-02-06 | Submitted: Saba onboarding, invoicing rollout, CQC delays. Missed 2026-02-13 (off Friday). |
| Informal Check-in | 2026-02-13 | Grant observation notes — positive and concerning items |

**Frequency**:
- 1:1 cadence: Weekly (Wednesday)
- Flash report submission rate: 1 of 2 weeks (50%) — missed week 2 due to Friday absence
- Unplanned escalations (last 30 days): 0

---

## 3. Feedback Register

### Feedback Given
| Date | Topic | Type | Acknowledged | Follow-up |
|------|-------|------|-------------|-----------|
| 2026-02-11 | Floor walks, snag list, reception vs floor role | Developmental | Yes | Complying as of 2026-02-13 |
| 2026-02-11 | Asana work board adoption | Developmental | Yes | Set up pending |
| 2026-02-13 | Exam bed not actioned, failure to communicate blocker | Corrective | Yes — apologised | Monitor |
| 2026-02-13 | Missed flash report (off Friday, didn't send before leaving) | Corrective | Pending — needs to be addressed |

### Feedback Received (from this person)
| Date | Topic | Context | Action Taken |
|------|-------|---------|-------------|
| 2026-02-06 | Maria Vega contract — requesting confirmation on change from fixed term to permanent | Flash report ask | Pending response |

### Feedback About (from third parties)
| Date | Source | Topic | Severity | Action Taken |
|------|--------|-------|----------|-------------|
| 2026-02-13 | Grant (observation) | Exam bed not cleared, missed flash report, medication delivery not communicated | High | To be addressed — Grant considering documented conversation |
| 2026-02-10 | CEO (James) | Recurring forgetfulness — multiple operational items not attended to | High | Expectations session held 2026-02-11 |
| 2026-02-10 | Yaseen (handover) | Open items being managed — Maria Vega contract, CQC review, facilities | Info | Tracked as tasks |

---

## 4. Task & Assignment Tracking

**Open Assignments**:
- [ ] Daily floor walks with snag list — assigned 2026-02-11, ongoing (complying)
- [ ] Asana work board adoption — assigned 2026-02-11, not started
- [ ] Maria Vega employment contract — close off draft (from Yaseen handover)
- [ ] Maria Vega room rental request — explore private consulting arrangement
- [ ] Maria Tam periodic review — 6-monthly check on private practice arrangement
- [ ] CQC policy annual review — in progress, delayed by busy week (2026-02-06)
- [ ] Plant wall installation — closing off with Plant Care
- [x] Lexee salary request — resolved 2026-02-18: 18% increase granted and communicated by Grant
- [ ] HBOT signage — up but needs images/graphics
- [ ] UK breathwork room -> immersive sensory room conversion — in progress, unclear instructions
- [ ] Shelving for store room and studio — to be ordered
- [ ] Glass board for consulting room — procuring alternative (previous too heavy for drywall)
- [ ] Water dispenser for waiting room — sourcing
- [ ] Ventilation/cooling for HBOT room and pharmacy store — costly, needs landlord approval
- [ ] Delivery management process — implement logging and communication for incoming packages
- [ ] Invoicing system go-live — continue supporting team confidence
- [ ] Clinical Governance meeting prep — agenda, information collation
- [ ] Saba ongoing training — working with Lexee
- [ ] **Second receptionist replacement** — hire urgently, previous resigned within 2 weeks (assigned 2026-02-18, URGENT)
- [ ] **Lexee role clarification** — assist in defining Lexee's role (receptionist/PA/liaison clarity) (assigned 2026-02-18, URGENT)

**Recently Completed** (last 30 days):
- [x] Flash report submission process — first report submitted 2026-02-06
- [x] Nursing roster — submitted 2026-02-10
- [x] Saba week one induction — completed 2026-02-06
- [x] Heating issues resolved (2026-02-06)
- [x] Lift shaft cleaning — done (check if repaint needed)
- [x] Exam bed area cleared — completed 2026-02-13 (with Grant's help)

**Patterns**:
- On-time completion: Inconsistent — responds to direct instructions but misses items without reminders; fails to escalate when blocked
- Communication on blockers: Poor — "oopsie" response when reminded; exam bed example shows she tries but doesn't communicate when stuck
- Flash report compliance: Missed when off on Friday — needs explicit process for absence days

---

## 5. Development & Growth

**Strengths Being Leveraged**:
- High commitment — first in, last out, long hours
- Cares deeply about the business
- Strong on CQC compliance — holds the CQC license
- Responsive to direct, clear instructions (floor presence compliance within days)
- Invoicing rollout showing operational capability when focused

**Growth Areas**:
- **Proactive attention to detail**: Picking up issues without being told — progress: expectations set 2026-02-11, mixed results
- **Task follow-through**: Completing assigned items without repeated reminders — progress: ongoing concern
- **Communication on blockers**: Escalating when stuck rather than going silent — progress: exam bed incident 2026-02-13
- **Prioritisation**: Working the floor vs sitting at reception — progress: complying as of 2026-02-13
- **Forward thinking**: Handover planning for days off, anticipating needs — progress: not yet demonstrated

**Skill Development Timeline**:
| Date | Milestone |
|------|-----------|
| 2026-02-11 | Expectations-setting session held — floor presence, snag list, work board |
| 2026-02-06 | First flash report submitted — good quality content |

---

## 6. Flash Report Trends

**Submission Rate**: 1 of 2 weeks (50%) — missed 2026-02-13 due to Friday absence
**Reports on file**: 1 (2026-02-06)

**Recurring Themes**:
- Accomplishments: Onboarding, invoicing system, facilities fixes
- Blockers: Busy weeks causing delays to planned work (CQC policies)
- Asks: Contract decisions (Maria Vega)

**Sentiment Trend**: Positive and engaged in submitted report. Closing line "Looking forward to welcoming you to the London Clinic" shows warmth.

---

## 7. Concerns & Patterns

**Active Concerns**:
1. **Recurring forgetfulness / lack of attention to detail**
   - First observed: 2026-02-10 (CEO feedback covering extended period)
   - Occurrences: Multiple — deliveries mismanaged, equipment stored improperly, clinic items not actioned, exam bed not cleared, medication delivery not communicated
   - Addressed: Yes — expectations session 2026-02-11, further incidents 2026-02-13
   - Status: Ongoing — Grant considering documented conversation (strike 2 of 3)

2. **Communication failures when blocked**
   - First observed: 2026-02-13 (exam bed incident)
   - Pattern: Tries to complete task, gets stuck, goes silent instead of escalating
   - Addressed: Yes — Grant discussed directly
   - Status: Monitoring

3. **Flash report compliance on absence days**
   - First observed: 2026-02-13
   - Issue: Did not send flash report before Friday absence, no handover info provided
   - Addressed: Pending — needs to be explicitly communicated
   - Status: To be addressed

4. **Delivery management**
   - First observed: 2026-02-13 (medication incident), noted as recurring
   - Issue: Packages received and stored without communicating to anyone
   - Impact: CEO could not find critical medications; potential patient safety and financial risk
   - Status: Process needs to be implemented

**Leading Indicators to Watch**:
- Flash report submission timing (compliance with Friday 4pm deadline, especially on absence weeks)
- Snag list activity (evidence of daily floor walks)
- Task completion without reminders
- CEO satisfaction during London visits
- Communication quality when blocked on tasks
- Delivery logging and notification

---

## 8. Key Decisions & Context

### Decisions Involving This Person
| Date | Decision | Rationale | Outcome |
|------|----------|-----------|---------|
| 2026-02-13 | Grant considering documented conversation if another incident occurs (strike 3) | Pattern of missed tasks despite expectations session | Pending — monitoring |
| 2026-02-11 | Set explicit floor walk and reception expectations | CEO feedback + forgetfulness pattern | Complying |
| 2026-02-11 | Introduce Asana work board | Need structured task tracking | Not started |

### Context for Future Reference
- UK-based — meetings at 11am+ SAST (Wednesday weekly)
- Holds CQC license for London clinic — critical compliance dependency
- Start date: 01 November 2024 (15 months in role); probation should have been completed ~Feb 2025 — check if written confirmation was issued
- Notice period: 3 months (post-probation)
- Yaseen recently handed over London operational oversight to Amanda (batch 1 & 2 items)
- CEO acknowledges Amanda's commitment and effort despite frustrations
- Instructions have sometimes been unclear — acknowledged in expectations session
- Receptionists (Saba + TBC) should man reception independently; Amanda as backup only
- Grant is frustrated by need to address issues daily — concerned about negative trajectory
- Maria Vega contract change (fixed to permanent) pending Grant's confirmation

---

## 9. Relationship Network

**Reports To**: Grant Merwitz (MD)
**Direct Reports**: Saba (Receptionist), second receptionist TBC
**Key Stakeholders**:
- James (CEO) — relationship: strained — frustrated by forgetfulness but acknowledges effort
- Yaseen (former operational oversight) — handover completed
- Elena Hilton — Amanda is Elena's non-clinical reporting line
- Lexee — training Saba, salary review pending

**Collaboration Quality**:
- With CEO: Strained — ongoing frustration despite acknowledged effort
- With clinical staff: Manages nursing roster, coordinates with Elena's team, invoicing rollout going well
- Cross-location: London-based, limited cross-location interaction

---

## 10. Source Files

| Resource | Location |
|----------|----------|
| Profile | `profile.md` |
| Performance Log | `performance.md` |
| 1:1 Notes | `one-on-ones/2026-02-11.md` |
| Flash Reports | `flash-reports/2026-02-06.md` |
| Documents | `documents/` |
| Updates | `updates/2026-02-11-*.md` (5 files: role expectations, work board tasks, post-meeting email, walkthrough checklist, role expectations attachment) |

---

*This information base is maintained by the inbox-processor agent. Manual edits are fine but may be overwritten during processing. For authoritative chronological records, see `performance.md`.*
