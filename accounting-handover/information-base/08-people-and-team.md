# People and Team

## Executive Leadership

### James La Porta - CEO & Lead Doctor

| Attribute | Detail |
|-----------|--------|
| **Role** | Chief Executive Officer, Lead Doctor |
| **Position** | Founder, Main Director |
| **Location** | South Africa (Cape Town) |

**Current Financial Responsibilities:**
- Only person with Level 4 banking access (Investec)
- Sole authority to release SARS payments (VAT, Income Tax)
- Final approval on major financial decisions
- Trust payment instructions flow through him

**Transition Relevance:**
- Critical dependency for SARS payment releases
- Must be kept informed of cash flow status
- Cannot be bypassed for tax-related payments

---

### Grant Merwitz - Managing Director

| Attribute | Detail |
|-----------|--------|
| **Role** | Managing Director |
| **Previous Role** | Chief Technology Officer |
| **Position** | Recently appointed to MD role |
| **Location** | South Africa |

**Current Financial Responsibilities:**
- Strategic oversight
- Transition project lead
- OCFO engagement discussions

**Transition Relevance:**
- Leading the handover process
- Primary contact for Alida during notice period
- Decision-maker on transition approach
- Technology background may help with systems/Xero aspects

---

### Yaseen Harneker - Chief Project Officer

| Attribute | Detail |
|-----------|--------|
| **Role** | Chief Project Officer |
| **Position** | Exco member |
| **Location** | South Africa |

**Current Financial Responsibilities:**
- Banking access (Investec) - can load and release non-SARS payments
- High-level strategy and decision-making with Grant and James
- Part of dual-control for payments

**Transition Relevance:**
- **Key backup for payment operations** - already has banking access
- Can take on increased payment loading/releasing role
- Exco involvement means strategic oversight capability
- Potential to absorb some of Alida's payment functions

**Capacity Consideration:** Already has CPO responsibilities - additional financial work may stretch capacity

---

## Practice Management

### Kyle Bennett - Practice Manager (Cape Town)

| Attribute | Detail |
|-----------|--------|
| **Role** | Practice Manager |
| **Location** | Cape Town, South Africa |
| **Branch** | SA operations |

**Current Financial Responsibilities:**
- Day-to-day practice operations
- Likely involved in patient-facing activities

**Transition Relevance:**
- **Rollout Lead (CPT)** for invoicing transformation
- **Process ownership** - governance, training, compliance - *not execution*
- Nurses and reception do the invoicing; Kyle ensures process runs correctly
- Escalation point for debtor disputes ("I didn't receive this")
- Local presence for SA operations coordination

---

### Amanda Ross - Practice Manager (UK)

| Attribute | Detail |
|-----------|--------|
| **Role** | Practice Manager |
| **Location** | United Kingdom |
| **Branch** | UK operations (Sanctura Ltd) |

**Current Financial Responsibilities:**
- Provides payroll changes to Alida/Lauren
- Has Barclays credit card (600 GBP limit)
- Day-to-day UK practice operations

**Transition Relevance:**
- **Rollout Lead (UK)** for invoicing transformation
- **Process ownership** - governance, training, compliance - *not execution*
- Nurses and reception do the invoicing; Amanda ensures process runs correctly
- Escalation point for debtor disputes ("I didn't receive this")
- Works with Kinari on UK financial matters
- Local UK coordination point

---

## Directors / Advisors

### Ricardo Delgado - Director

| Attribute | Detail |
|-----------|--------|
| **Role** | Director (less active operationally) |
| **Involvement** | Financial oversight, targets review |
| **Relationship** | Worked closely with Alida |
| **Location** | Remote - not physically in Cape Town |

**Current Financial Responsibilities:**
- Close review of financial targets and state
- Involved in financial discussions
- Steps up to help when needed

**Transition Relevance:**
- **Potential increased involvement** during transition
- Has financial acumen and context from working with Alida
- Could provide oversight/review of financial reporting
- May be able to bridge some knowledge gaps
- Understands historical financial picture
- **Note:** As "external" (not in CPT), needs structured engagement like Kinari

**Key Asset:** Relationship with Alida means he has context others may lack

---

## Departing Staff

### Alida Wiese - Financial Manager (Departing)

| Attribute | Detail |
|-----------|--------|
| **Role** | Financial Manager |
| **Tenure** | Many years |
| **Resignation Date** | 26 January 2026 |
| **Last Day** | 23 February 2026 |

**Responsibilities Being Handed Over:**
- See [02-entities-and-responsibilities.md](02-entities-and-responsibilities.md) for full scope
- Invoicing, supplier payments, payroll data, compliance, reporting, banking operations

---

## External Service Providers

### Inaura - Accounting Practice (South Africa)

| Attribute | Detail |
|-----------|--------|
| **Type** | Accounting practice |
| **Current Services** | Payroll, PAYE, VAT, Income Tax, Financial Statements |
| **Entities Served** | SA entities (Inc, Saint Raphael, TTC) |
| **Service Quality** | Questionable historically |

**Current Scope:**
| Service | Frequency |
|---------|-----------|
| Payroll processing | Monthly |
| Payslip generation | Monthly |
| PAYE submission | Monthly |
| VAT calculation & submission | Bi-monthly |
| Financial statements | Annual |
| Income tax returns | Annual |

**Transition Relevance:**
- Already embedded in SA financial operations
- **Continues current obligations** (VAT, payroll, PAYE, income tax) - no phase-out during transition
- Could potentially **expand scope** to cover more of Alida's work
- Would need to "up their game" - service quality concerns exist
- Relationship needs assessment - are they capable of more?

**Questions to Address:**
1. Can Inaura take on bookkeeping/reconciliation work?
2. Can they improve responsiveness and quality?
3. What would expanded scope cost?
4. Do we trust them with more responsibility given past performance?

**Note:** Avoid adding new providers (e.g., Imperium for additional services) during transition unless determined essential. Focus on stabilizing existing relationships first.

---

### Kinari - Accounting Support (UK)

| Attribute | Detail |
|-----------|--------|
| **Type** | Accounting/bookkeeping support |
| **Current Services** | Banking, payroll, financial statements |
| **Entities Served** | UK entities (Sanctura Ltd, Holdings) |

**Current Scope:**
- Complete management of UK Holding Company
- Banking support for Sanctura Ltd
- Payroll processing with Lauren
- PAYE submission
- Financial statements and income tax

**Transition Relevance:**
- UK operations relatively well-covered by Kinari
- Less disruption expected in UK vs SA
- Could potentially take on more UK-side work if needed

---

### OCFO / Fractional CFO Services (Potential)

| Attribute | Detail |
|-----------|--------|
| **Type** | Consultancy - can provide various services |
| **Status** | In discussions - Grant meeting candidates |
| **Potential Services** | Fractional CFO, bookkeeper, or both |

**Clarification:** OCFO is a consultancy that can provide different resources based on need. Not just a CFO service - they can provide a bookkeeper for day-to-day operations while also offering CFO-level oversight if needed.

**Potential Scope:**

| Service | Suitability |
|---------|-------------|
| Interim financial management | High |
| Bookkeeping and reconciliation | High |
| Management accounts preparation | High |
| Cash flow monitoring | High |
| Supplier payment management | Medium |
| Invoicing operations | Medium |
| Compliance oversight | Medium |
| Process documentation | High |
| Invoicing transformation project leadership | High |

**Transition Relevance:**
- **Primary external resource** for bridging the gap
- Can provide immediate professional capacity
- Brings external expertise and best practices
- Scalable - can flex up/down as needed
- Can help implement invoicing transformation project
- Potential to establish better processes than before

**Key Decisions:**
1. Scope of engagement (what specifically do they cover?)
2. Duration (interim only, or longer term?)
3. Handover plan (to new hire or permanent arrangement?)
4. Cost vs value assessment

---

## Team Capacity Matrix

### Who Can Do What?

| Activity | Current Owner | Potential Backup | External Option |
|----------|---------------|------------------|-----------------|
| SA invoicing (Healthbridge) | Alida | Candice (some exp) | OCFO |
| SA invoicing (Xero) | Alida | OCFO | OCFO |
| UK invoicing | Alida | Kinari | Kinari |
| SA supplier payments - load | Alida | OCFO | Grant |
| SA supplier payments - release | Alida/Yaseen | Yaseen | - |
| SARS payments - release | James only | None | None |
| UK payments | Alida/Kinari | Kinari | Kinari |
| SA payroll data | Alida | ? | OCFO |
| SA payroll processing | Inaura | Inaura | Inaura |
| UK payroll | Kinari/Lauren | Kinari | Kinari |
| Management accounts | Alida | Ricardo (review) | OCFO |
| Weekly KPIs | Alida | ? | OCFO |
| Annual forecast | Alida | ? | OCFO |
| Bank monitoring | Alida | Yaseen | OCFO |
| Debtors chasing | Alida | Escalation model (see plan) | OCFO |
| Compliance calendar | Alida | ? | OCFO/Inaura |
| Landlord relations | Alida | Grant? | - |
| Insurance liaison | Alida | Grant? | - |
| Trust instructions | Alida | Jess (intro planned) | - |

### Capacity Assessment

| Person | Current Load | Available Capacity | Best Suited For |
|--------|--------------|-------------------|-----------------|
| James | High (CEO + Clinical) | Very Limited | SARS releases, approvals |
| Grant | High (MD transition) | Limited | Oversight, decisions, relationships |
| Yaseen | High (CPO) | Limited | Payment operations, banking |
| Kyle | Medium-High | Some | Practice-level invoicing, local coordination |
| Amanda | Medium-High | Some | UK practice operations |
| Ricardo | Variable | Moderate | Financial oversight, reporting review |
| Candice | ? | Some | Healthbridge invoicing support |

---

## Recommended Role Allocation (Transition Period)

### Internal Team

| Person | Additional Responsibilities |
|--------|---------------------------|
| **Yaseen** | Primary banking operations (loading, non-SARS releases), cash flow alerts |
| **Ricardo** | Financial reporting review, liaison with OCFO, Alida knowledge capture |
| **Kyle** | SA invoicing governance (not execution), debtor disputes (SA), stock count |
| **Amanda** | UK invoicing governance (not execution), debtor disputes (UK), Kinari coordination |
| **Grant** | Overall transition oversight, OCFO management, strategic decisions |
| **James** | SARS payment releases, major approvals (minimise additional load) |

### Payment Controls (During Transition)

**Dual control must be maintained:**
- **Bookkeeper/OCFO:** Loads payments (no release authority)
- **Grant or Yaseen:** Releases payments (segregation from loader)
- **James:** SARS payments only (VAT, Income Tax)

This ensures no single person controls the full payment process. Even during transition with new/interim staff, controls remain in place.

### External Support

| Provider | Scope |
|----------|-------|
| **OCFO** | Bookkeeping, management accounts, cash flow monitoring, process documentation, invoicing project support |
| **Inaura** | Continue current scope; assess for expansion if quality improves |
| **Kinari** | Continue current scope; potential slight expansion for UK |

---

## Communication and Reporting Lines

### Current State (with Alida)
```
James (CEO)
    ↓
Alida (Financial Manager) ←→ Ricardo (oversight)
    ↓
Inaura / Kinari / Service Providers
```

### Proposed Transition State
```
James (CEO) ─────────────────────────────────┐
    ↓                                         │
Grant (MD) ──────→ OCFO (interim support)     │
    ↓                    ↓                    │
Yaseen (banking ops)  Bookkeeping/Reporting   │
    ↓                    ↓                    │
Inaura ←─────────── Coordination ──────→ Ricardo (review)
Kinari                                        │
                                              ↓
                                    SARS releases (James only)
```

---

## Key Contacts Summary

| Name | Role | Primary Transition Function |
|------|------|---------------------------|
| James La Porta | CEO | SARS releases, final approvals |
| Grant Merwitz | MD | Transition lead, OCFO engagement |
| Yaseen Harneker | CPO | Banking operations backup |
| Kyle Bennett | Practice Mgr (CPT) | SA local coordination |
| Amanda Ross | Practice Mgr (UK) | UK local coordination |
| Ricardo Delgado | Director | Financial oversight, knowledge bridge |
| Alida Wiese | Financial Mgr (departing) | Knowledge transfer source |
| Inaura | Accountants | SA payroll, tax, compliance |
| Kinari | Accountants | UK operations |
| OCFO (TBC) | Consultancy | Interim financial support |
