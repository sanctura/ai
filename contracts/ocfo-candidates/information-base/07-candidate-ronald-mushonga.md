# Candidate Profile: Ronald Mushonga

**Source:** LVB Consulting (Lushia)
**Date Added:** 3 February 2026
**Status:** Contract received (Draft One) - starting 10 Feb 2026, background checks passed

---

## Quick Facts

| Field | Value |
|-------|-------|
| **Name** | Ronald Faku Mushonga |
| **Qualification** | CA(SA), ACA, CPA, TPPE |
| **Experience** | 15+ years |
| **Location** | Johannesburg, South Africa |
| **Contact** | +27 83 737 9891 / ronmush8@gmail.com |
| **LinkedIn** | linkedin.com/in/ronald-mushonga-4657138b |
| **Current Role** | Fractional CFO Consultant (Hatua Capital) |
| **Availability** | Immediate (per LinkedIn post) |
| **Rate** | R115,000/month (ex VAT) - final negotiated |
| **Agency Fee** | R55,200 (12% of 4-month contract value) |
| **Total 4-month cost** | R515,200 (excl return flight) |
| **Company** | Clearshell Management Solutions (Pty) Ltd (Reg: 2018/272505/07) |
| **Tax Status** | SARS compliant - clearance valid to 5 Feb 2027 |

---

## Professional Experience

| Period | Role | Company | Key Achievements |
|--------|------|---------|------------------|
| Jan 2022 – Present | Fractional CFO | Hatua Capital | Multiple engagements (see below) |
| Jan 2019 – Dec 2021 | Head of Finance | Air Chefs SOC Ltd (SAA Group) | Raised R30m; navigated business rescue; led team of 12 |
| Mar 2012 – Aug 2018 | Financial Director | Huge Telecom (JSE-listed) | Chaired ExCo; led M&A; JSE compliance |
| Feb 2008 – Jan 2012 | Audit Manager | BDO Inc / PKF Inc | Board exams in one sitting; IFRS mastery |

### Recent Fractional CFO Engagements

**RDG Collective (Mar 2024 – Aug 2025)**
- Raised >$10m equity/debt
- Rebuilt finance team
- Uncovered fraud
- Implemented governance policies

**Broadband Infraco SOC Ltd (Jan – Feb 2024)**
- Led year-end audit prep
- Reviewed financials & systems
- Ensured IFRS/Treasury compliance

---

## Education

| Qualification | Institution | Year |
|---------------|-------------|------|
| CA(SA), ACA, CPA | - | - |
| BCompt Honours (Accounting & Finance) | UNISA | 2008 |
| BCom (Accounting) | University of the Witwatersrand | 2007 |
| TPPE | UCT (accredited) | - |
| Cambridge A-Levels | Christian Brothers College, Bulawayo | 2001 |

---

## Core Competencies

- Capital Raising (Financial Strategy & Modelling)
- Governance (Financial Reporting & Compliance - JSE, IFRS)
- Leadership (Transformation & Team Building)

---

## Fit Assessment (Preliminary)

### Potential Strengths for Sanctura

| Requirement | Ronald's Experience |
|-------------|---------------------|
| Multi-entity experience | JSE-listed group, SOC subsidiaries |
| Crisis management | Business rescue experience (SAA) |
| Team leadership | Led team of 12; rebuilt finance teams |
| Governance | JSE compliance, IFRS, Auditor General audits |
| Capital/funding | Raised >$10m equity/debt; R30m from DPE |
| Fractional/interim | Currently operating as fractional CFO |

### Interview Findings (4 Feb 2026)

| Question | Answer from Interview |
|----------|----------------------|
| Xero experience? | Not discussed - still unknown |
| Healthcare sector? | No - but not a blocker |
| Availability | Confirmed immediate |
| Rate | R110k/month (negotiable to R100k for longer term) |
| Multi-country | Not specifically addressed |
| Hands-on vs strategic? | **Yes, willing** - but needs junior support to be effective |
| Technical adaptability | Self-rated 8-10/10 |
| Cash flow approach | 13-week rolling forecast (more robust than Keith's 30-day) |

### Outstanding Negotiations

| Item | Issue |
|------|-------|
| Work arrangement | Proposed 2 weeks Cape Town / 2 weeks remote - Grant doesn't like this |
| Travel costs | Does R110k include flights/accommodation from Joburg? |
| Rate negotiation | Potentially R100k for 6-month contract |

---

## Comparison to Keith Harrod

| Factor | Keith (OCFO) | Ronald (LVB) |
|--------|--------------|--------------|
| Qualification | CA(SA) | CA(SA), ACA, CPA |
| Experience | 15+ years | 15+ years |
| Multi-entity | Yes (Janetta - 12 entities) | Yes (JSE groups, SOCs) |
| Business rescue | Not mentioned | Yes (SAA) |
| Team building | Will help hire bookkeeper | Detailed structure + plan |
| Cash flow method | 30-day rolling weekly | **13-week rolling** |
| Hands-on | Confirmed | Confirmed (with support) |
| Xero | Unclear | Unknown |
| Healthcare | No | No |
| Location | Cape Town (local) | **Joburg (travel required)** |
| Rate | R110k/month | R110k/month (negotiable) |
| Agency fee | Included | +R60,720 (12%) |
| **4-month total** | **R440,000** | **~R500,720 + travel?** |
| Permanent hire fee | ~R220k (240hrs penalty) | ~R152k (10% CTC) |
| Restriction period | 24 months | 18 months |
| **Ricardo's verdict** | "Could do the job" | **"Preferred - more on the pulse"** |

---

## Interview Progress

| Stage | Date | Attendees | Status |
|-------|------|-----------|--------|
| Intro interview | 3 Feb 2026 | Grant | Completed |
| Full interview | 4 Feb 2026 | Ricardo, James, Grant | **Completed** |

---

## Next Steps

- [x] Confirm rate with Lushia - R110k/month confirmed
- [x] Interview with Ricardo - **Completed, preferred candidate**
- [x] Confirm hands-on approach - Yes, with support requirement
- [x] Negotiate work arrangement - settled at R115k all-in (Ronald covers own travel/accommodation)
- [x] Clarify travel costs - included in R115k rate, we only cover Feb return flight
- [x] Final cost comparison vs Keith - completed
- [x] Make decision - **Ronald selected**
- [x] Background checks - **passed, no concerns (8 Feb 2026)**
- [x] Draft contract received - **Draft One received 8 Feb 2026**
- [ ] Grant reviews contract (yellow-highlighted sections)
- [ ] Ronald signs agreement via OptiWorX
- [ ] **Ronald starts: Tuesday, 10 Feb 2026**

---

## References

Available on request - LinkedIn recommendations from CEOs of RDG Collective and Huge Group.
